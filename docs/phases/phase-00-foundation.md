...
describe('collectPackages traversal', () => {
  ...
  it('records both versions when two dependents resolve to different copies', async () => {
    ...
    await writePkg('node_modules/gamma/package.json', { name: 'gamma', version: '1.0.0', license: 'MIT', dependencies: { beta: '^2.0.0' } });
    await writePkg('node_modules/beta/package.json', { name: 'beta', version: '2.0.0', license: 'MIT' });
    await writePkg('node_modules/alpha/node_modules/beta/package.json', { name: 'beta', version: '1.0.0', license: 'GPL-3.0' });

    const packages = await collectPackages(dir);
    expect(packages.filter((p) => p.name === 'beta').map((p) => p.version).sort()).toEqual(['1.0.0', '2.0.0']);
