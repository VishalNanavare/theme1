...
**Architecture:** Nunjucks templates in `src/pages/` are rendered to flat HTML in `src/.gen/` by `scripts/render-pages.mjs`. Vite's root **is** `src/.gen`, so its multi-page build emits `dist/*.html` flat, mirroring the deliverable shape. Pages reference styles and scripts with relative URLs (`../styles/…`, `../scripts/…`), which Vite bundles, hashes, and rewrites. A Vite plugin re-renders pages on `.njk` change in dev.
...
| `vite.config.js` | MPA config, root `src/.gen`, SCSS/PostCSS options |
...
| `.gitignore` | `node_modules`, `dist`, `src/.gen` |
...
- Produces:
  ...
  - Constants `SRC = 'src'`, `PAGES_DIR = 'src/pages'`, `OUT_DIR = 'src/.gen'`, `DATA_DIR = 'src/data'`
...
`src/layouts/base.njk` — pages live flat in `src/.gen/`, so `../` always reaches `src/`:
...
Expected: `rendered 1 pages`, and `src/.gen/index.html` exists containing `theme1 0.1.0`.
...
```json
{
  ...
  "ignoreFiles": ["**/node_modules/**", "dist/**", "src/.gen/**"]
}
```
...
- [ ] `npm run build` produces `dist/index.html` with hashed, rewritten asset URLs and no `.gen` directory.
